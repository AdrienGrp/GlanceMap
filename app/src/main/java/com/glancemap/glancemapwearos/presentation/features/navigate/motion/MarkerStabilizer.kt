...
internal class MarkerStabilizer(
    ...
    fun onGpsFix(
        ...
        if (currentRendered != null && lastGps != null && lastGpsElapsedMs > 0L) {
            ...
            if (dtMs >= minOutlierWindowMs) {
                ...
                val allowedJumpMeters =
                    (lastGpsAccuracyM + accuracy + OUTLIER_JUMP_MARGIN_M)
        ...
        val sinceLastGpsMs =
            if (lastGpsElapsedMs > 0L && fixElapsedMs > 0L) {
                (fixElapsedMs - lastGpsElapsedMs).coerceAtLeast(0L)
            } else {
                0L
            }
        val jumpToRenderedMeters =
            if (currentRendered != null) {
        ...
        val shouldClampStationaryDrift =
            currentRendered != null &&
...
private fun resolveWakeCorrectionMaxGapMs(minGapMs: Long): Long = (minGapMs * WAKE_CORRECTION_MAX_GAP_MULTIPLIER).coerceAtLeast(minGapMs)
