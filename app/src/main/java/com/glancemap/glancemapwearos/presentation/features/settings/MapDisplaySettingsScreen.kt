...
private fun timeFormatLabel(format: String): String = when (format) {
    SettingsRepository.TIME_FORMAT_24_HOUR -> "24-hour"
    SettingsRepository.TIME_FORMAT_12_HOUR -> "12-hour"
    else -> "24-hour"
}

private fun markerStyleLabel(style: String): String = when (style) {
    SettingsRepository.MARKER_STYLE_DOT -> "Dot + cone"
    SettingsRepository.MARKER_STYLE_TRIANGLE -> "Arrow"
    else -> "Dot + cone"
}

private fun northIndicatorModeLabel(mode: String): String = when (mode) {
    "ALWAYS" -> "Always"
    "COMPASS_ONLY" -> "Compass only"
    "NORTH_UP_ONLY" -> "North-up only"
    "NEVER" -> "Never"
    else -> mode.replace("_", " ").lowercase()
        .replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }
}

private fun zoomButtonsModeLabel(mode: String): String = when (mode) {
