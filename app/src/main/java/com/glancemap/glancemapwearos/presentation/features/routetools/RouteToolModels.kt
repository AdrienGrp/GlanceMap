...
internal enum class RouteToolKind(val title: String) {
    CREATE(title = "Create"),
    MODIFY(title = "Modify")
}
...
internal enum class RouteSaveBehavior(val title: String) {
    SAVE_AS_NEW(title = "Save as new"),
    REPLACE_CURRENT(title = "Replace current")
}

internal enum class LoopStartMode(val title: String) {
    CURRENT_LOCATION(title = "Current"),
    PICK_ON_MAP(title = "Pick on map")
}

internal enum class LoopTargetMode(val title: String) {
...
internal data class RouteToolOptions(
    ...
    val requiresRoutingData: Boolean
        get() = when (toolKind) {
            ...
            RouteToolKind.MODIFY -> when (modifyMode) {
                RouteModifyMode.RESHAPE_ROUTE,
                RouteModifyMode.REPLACE_SECTION_A_TO_B -> true
                RouteModifyMode.KEEP_ONLY_A_TO_B,
                RouteModifyMode.TRIM_START_TO_HERE,
                RouteModifyMode.TRIM_END_FROM_HERE,
                RouteModifyMode.REVERSE_GPX -> false
            }
        }
}

internal fun RouteToolOptions.withVisibleLoopDefaults(): RouteToolOptions {
    return copy(loopShapeMode = LoopShapeMode.PREFER_CIRCUIT)
}

internal enum class RouteSelectionTarget(val title: String) {
...
internal data class RouteToolSession(
    ...
    val isMultiPointCreate: Boolean
        get() = options.toolKind == RouteToolKind.CREATE &&
            options.createMode == RouteCreateMode.MULTI_POINT_CHAIN

    val currentSelectionTarget: RouteSelectionTarget?
        get() = when (options.toolKind) {
            RouteToolKind.CREATE -> when (options.createMode) {
                ...
                RouteCreateMode.POINT_A_TO_B -> when {
                ...
                RouteCreateMode.LOOP_AROUND_HERE -> when (options.loopStartMode) {
            ...
            RouteToolKind.MODIFY -> when (options.modifyMode) {
                RouteModifyMode.RESHAPE_ROUTE -> {
                    when {
                ...
                RouteModifyMode.REPLACE_SECTION_A_TO_B -> when {
                    pointA == null -> RouteSelectionTarget.POINT_A
                    pointB == null -> RouteSelectionTarget.POINT_B
                    else -> null
                }

                RouteModifyMode.KEEP_ONLY_A_TO_B -> when {
                    pointA == null -> RouteSelectionTarget.POINT_A
                    pointB == null -> RouteSelectionTarget.POINT_B
                    else -> null
                }
    ...
    val instructionText: String
        get() = when (currentSelectionTarget) {
            RouteSelectionTarget.RESHAPE_POINT -> "Place route point, then check."
            RouteSelectionTarget.DESTINATION -> when {
            ...
            RouteSelectionTarget.POINT_A -> if (
            ...
            RouteSelectionTarget.POINT_B -> if (options.toolKind == RouteToolKind.CREATE) {
    ...
    fun captureSelection(point: LatLong): RouteToolSession {
        return when (currentSelectionTarget) {
    ...
    fun captureTrackInsertion(point: LatLong): RouteToolSession {
        return if (currentSelectionTarget == RouteSelectionTarget.TRACK_INSERTION) {
            copy(pointA = point)
        } else {
            this
        }
...
private fun sameRouteToolLocation(a: LatLong, b: LatLong): Boolean {
    return kotlin.math.abs(a.latitude - b.latitude) < 1e-9 &&
        kotlin.math.abs(a.longitude - b.longitude) < 1e-9
}

internal fun LatLong.toRoutePointLabel(): String {
