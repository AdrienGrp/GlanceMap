...
class PoiRepositoryImpl(
    ...
    override suspend fun listPoiFiles(): List<File> = withContext(Dispatchers.IO) {
        if (!poiDir.exists()) return@withContext emptyList()
        poiDir.listFiles { _, name -> name.endsWith(".poi", ignoreCase = true) }
    ...
    override suspend fun deletePoiFile(path: String): Boolean = withContext(Dispatchers.IO) {
        val file = File(path)
        val isInsideDir = runCatching {
    ...
    override suspend fun readCategories(path: String): List<PoiCategory> = withContext(Dispatchers.IO) {
        ...
        openPoiDatabase(poiFile.absolutePath).use { db ->
            db.rawQuery(
                ...
                while (cursor.moveToNext()) {
                    ...
                    rawCategories += RawPoiCategory(
                        id = cursor.getInt(idIdx),
                        name = cursor.getString(nameIdx).orEmpty(),
                        parent = if (parentIdx >= 0 && !cursor.isNull(parentIdx)) {
            ...
            db.rawQuery(
                ...
                while (cursor.moveToNext()) {
                    ...
                    val pointCount = if (countIdx >= 0 && !cursor.isNull(countIdx)) {
        ...
        val combinedAliases = mutableMapOf<Int, Set<Int>>().apply {
        ...
        val sortWeights = buildCategorySortWeights(
    ...
    override suspend fun isFileEnabled(path: String): Boolean = withContext(Dispatchers.IO) {
        prefs.getBoolean(fileEnabledKey(path), false)
    }

    override suspend fun setFileEnabled(path: String, enabled: Boolean) = withContext(Dispatchers.IO) {
        prefs.edit()
    ...
    override suspend fun getEnabledCategories(
        ...
        val stored = prefs.getStringSet(key, emptySet()).orEmpty()
            ...
            .toSet()
        val aliasMap = synchronized(mergedCategoryAliasesByPath) {
        ...
        buildSet {
            stored.forEach { storedId ->
                ...
                val candidates = availableCategoryIds.filter { candidate ->
    ...
    override suspend fun countPoiPoints(
        ...
        val expandedCategoryIds = expandCategoryIdsWithMergedAliases(
    ...
    override suspend fun queryPoiPointsByCategories(
        ...
        categoryIds: Set<Int>,
        limit: Int
    ): List<PoiPoint> = withContext(Dispatchers.IO) {
        val poiFile = File(path)
        if (!poiFile.exists() || !poiFile.isFile) return@withContext emptyList()
        ...
        openPoiDatabase(poiFile.absolutePath).use { db ->
            db.rawQuery(sql, args).use { cursor ->
                ...
                while (cursor.moveToNext()) {
                    ...
                    val data = if (dataIdx >= 0 && !cursor.isNull(dataIdx)) {
            ...
            .sortedWith(
                compareBy<PoiPoint>(
                    ...
                    Comparator { left, right ->
                        ...
                        val rankCompare = poiSearchMatchRank(leftName, queryLower)
    ...
    override suspend fun deleteVisibilityState(path: String) = withContext(Dispatchers.IO) {
        prefs.edit()
    ...
    private fun openPoiDatabase(path: String): SQLiteDatabase {
        return SQLiteDatabase.openDatabase(path, null, SQLiteDatabase.OPEN_READONLY)
    }

    private fun fileEnabledKey(path: String): String {
        return KEY_FILE_ENABLED_PREFIX + File(path).name.lowercase(Locale.ROOT)
    }

    private fun enabledCategoriesKey(path: String): String {
        return KEY_ENABLED_CATEGORY_PREFIX + File(path).name.lowercase(Locale.ROOT)
    }
