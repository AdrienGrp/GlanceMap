...
internal fun queryDisplayRotation(windowManager: WindowManager): Int {
    return runCatching { windowManager.defaultDisplay.rotation }
        .getOrDefault(Surface.ROTATION_0)
}

internal fun shouldSampleDisplayRotation(nowElapsedMs: Long, lastSampleAtMs: Long): Boolean {
    return nowElapsedMs - lastSampleAtMs >= DISPLAY_ROTATION_SAMPLE_INTERVAL_MS
}
...
internal fun resolveCompassSensorAvailability(
    ...
    return CompassSensorAvailability(
...
internal fun startupHeadingPublishesToMaskForPipeline(
    ...
    return when (pipeline) {
        ...
        HeadingPipeline.MAG_ACCEL_FALLBACK -> STARTUP_HEADING_PUBLISH_MASK_SAMPLES_DEFAULT
...
internal fun prepareCompassRegistrationResetState(
    ...
    return CompassRegistrationResetState(
        headingRelockUntilElapsedMs = maxOf(
            currentHeadingRelockUntilElapsedMs,
            nowElapsedMs + HEADING_RELOCK_WINDOW_MS
        ),
        magneticInterferenceStartupGraceUntilElapsedMs = maxOf(
...
internal fun decodeRotationVectorUncertainty(
    ...
    val uncertaintyDeg = if (uncertaintyRad.isFinite() && uncertaintyRad >= 0f) {
...
internal fun buildCompassHeadingSampleLog(
    ...
    return "heading raw=${rawHeading.format(1)} smoothed=${smoothedHeading.format(1)} " +
