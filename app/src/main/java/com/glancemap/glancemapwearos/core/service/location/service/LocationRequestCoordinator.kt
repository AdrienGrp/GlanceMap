...
internal class LocationRequestCoordinator(
    ...
    fun requestLocationUpdateIfNeeded() {
        ...
        launchedJob = serviceScope.launch {
    ...
    private fun isSuperseded(generation: Long): Boolean {
        return generation != requestGeneration.get()
    }
    ...
    private fun resolveRequestPlan(
        ...
        val spec = engine.resolveRequestSpec(
    ...
    private fun scheduleRetry(
        ...
        scheduledRetryJob = serviceScope.launch {
            try {
                if (delayMs > 0L) {
                    delay(delayMs)
                }
...
private fun resolveRequestFailureRetryDelayMs(failureCount: Long): Long = when {
