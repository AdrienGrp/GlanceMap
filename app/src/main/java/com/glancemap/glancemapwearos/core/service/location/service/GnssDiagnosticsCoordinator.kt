...
internal class GnssDiagnosticsCoordinator(
        ...
        val callback = object : GnssStatus.Callback() {
            ...
            override fun onSatelliteStatusChanged(status: GnssStatus) {
                ...
                if (statusSampleCount == 1) {
                    val sinceRegisterMs = if (collectorRegisteredAtElapsedMs > 0L) {
    ...
    fun unregister(reason: String = "unspecified") {
        ...
        if (callback != null) {
            ...
            val nowElapsedMs = SystemClock.elapsedRealtime()
            val runtimeMs = if (collectorRegisteredAtElapsedMs > 0L) {
                (nowElapsedMs - collectorRegisteredAtElapsedMs).coerceAtLeast(0L)
            } else {
                -1L
            }
            val lastStatusAgeMs = if (lastStatusAtElapsedMs > 0L) {
    ...
    private fun scheduleStatusWatchdogIfNeeded() {
        ...
        statusWatchdogJob = serviceScope.launch {
    ...
    private fun gpsProviderPresent(manager: LocationManager?): Boolean {
        return runCatching { manager?.allProviders?.contains(LocationManager.GPS_PROVIDER) == true }
            .getOrDefault(false)
    }

    private fun gpsProviderEnabled(manager: LocationManager?): Boolean {
        return runCatching { manager?.isProviderEnabled(LocationManager.GPS_PROVIDER) == true }
            .getOrDefault(false)
    }
}
