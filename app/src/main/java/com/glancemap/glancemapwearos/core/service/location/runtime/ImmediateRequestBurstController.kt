...
internal class ImmediateRequestBurstController(
    ...
    sealed interface Decision {
        data class Start(val burstId: Long) : Decision
        data class SkipCooldown(val remainingMs: Long) : Decision
        data class SkipActiveBurst(val activeBurstId: Long) : Decision
    }
    ...
    fun request(nowElapsedMs: Long): Decision = synchronized(lock) {
    ...
    fun end(expectedBurstId: Long? = null): Long? = synchronized(lock) {
