...
object BundledRenderThemeAssetLocator {
    ...
    private val specs: Map<String, ThemeSpec> = mapOf(
        MapsforgeThemeCatalog.ELEVATE_THEME_ID to ThemeSpec(
            generatedRoot = "theme/elevate",
            xmlFileName = "Elevate.xml",
            legacyXmlPath = "Elevate.xml",
            legacyResourceRoot = "ele-res"
        ),
        MapsforgeThemeCatalog.ELEVATE_WINTER_THEME_ID to ThemeSpec(
            generatedRoot = "theme/elevate-winter",
            xmlFileName = "Elevate.xml"
        ),
        MapsforgeThemeCatalog.ELEVATE_WINTER_WHITE_THEME_ID to ThemeSpec(
            generatedRoot = "theme/elevate-winter-white",
            xmlFileName = "Elevate.xml"
        ),
        MapsforgeThemeCatalog.OPENHIKING_THEME_ID to ThemeSpec(
            generatedRoot = "theme/openhiking",
            xmlFileName = "OpenHiking.xml"
        ),
        MapsforgeThemeCatalog.FRENCH_KISS_THEME_ID to ThemeSpec(
            generatedRoot = "theme/frenchkiss",
            xmlFileName = "frenchkiss.xml"
        ),
        MapsforgeThemeCatalog.TIRAMISU_THEME_ID to ThemeSpec(
            generatedRoot = "theme/tiramisu",
            xmlFileName = "Tiramisu.xml"
        ),
        MapsforgeThemeCatalog.HIKE_RIDE_SIGHT_THEME_ID to ThemeSpec(
            generatedRoot = "theme/hike-ride-sight",
            xmlFileName = "HikeRideSight.xml"
        )
    )

    fun isThemeAvailable(assets: AssetManager, themeId: String): Boolean {
    ...
    internal fun resolveKnownThemeIdOrNull(themeId: String?): String? {
        return when (themeId?.trim()) {
            null,
            "",
            MapsforgeThemeCatalog.ELEVATE_THEME_ID -> MapsforgeThemeCatalog.ELEVATE_THEME_ID
    ...
    private fun requireKnownThemeId(themeId: String?): String {
        return requireNotNull(resolveKnownThemeIdOrNull(themeId)) {
    ...
    private fun directoryLooksPresent(assets: AssetManager, path: String): Boolean {
        return runCatching { assets.list(path)?.isNotEmpty() == true }
            .getOrDefault(false)
    }
}
