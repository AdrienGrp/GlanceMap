...
internal class HttpTransferProgressCallbacks(
    ...
    private val uiThrottler = UiUpdateThrottler(
        minIntervalMs = 500L,
        minStepBytes = if (isMapLikeTransferFile(metadata.fileName)) {
            8L * 1024 * 1024
        } else {
            1L * 1024 * 1024
        }
    ...
    fun onTransferState(phase: String, detail: String) {
        if (detail.isNotBlank()) {
            val progress = if (metadata.totalSize > 0L) {
    ...
    fun onProgress(bytesCopied: Long) {
        ...
        if (uiThrottler.shouldUpdate(bytesCopied)) {
            val progress = if (metadata.totalSize > 0) {
