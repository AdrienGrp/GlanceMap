...
object TransferUtils {
    ...
    private val sha256Cache = object : LinkedHashMap<String, String>(SHA_CACHE_MAX_ENTRIES, 0.75f, true) {
        override fun removeEldestEntry(eldest: MutableMap.MutableEntry<String, String>?): Boolean {
            return size > SHA_CACHE_MAX_ENTRIES
        }
    }

    fun getFileDetails(context: Context, uri: Uri): Pair<String, Long>? {
        val queried = context.contentResolver.query(uri, null, null, null, null)?.use { cursor ->
        ...
        try {
            while (true) {
                ...
                if (timeDelta >= UPDATE_INTERVAL_MS ||
                    ...
                    val speedMBps = if (timeDelta >= SPEED_WARMUP_MIN_MS) {
    ...
    internal fun scoreLocalIpCandidate(ifName: String, ip: String): Int {
    ...
    private fun queryShaCacheMetadata(context: Context, uri: Uri): ShaCacheMetadata {
        return runCatching {
            context.contentResolver.query(uri, null, null, null, null)?.use { cursor ->
                ...
                val lastModified = if (modifiedIndex != -1 && !cursor.isNull(modifiedIndex)) {
    ...
    private fun formatHashProgress(copied: Long, total: Long?): String {
