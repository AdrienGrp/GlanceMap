...
internal class PhoneDataLayerRepository(
    ...
    private val _events = MutableSharedFlow<PhoneDataLayerEvent>(
        replay = 0,
    ...
    suspend fun sendCancelTransfer(nodeId: String, transferId: String) {
        val payload = JSONObject().apply { put("id", transferId) }
            .toString()
    ...
    private fun updateWatchList(nodes: Set<Node>) {
        val mapped = nodes
            .mapNotNull { node ->
                val nodeId = runCatching { node.id }
                    .getOrNull()
                    ?.trim()
                    .orEmpty()
                if (nodeId.isBlank()) return@mapNotNull null

                val displayName = runCatching { node.displayName }
                    .getOrNull()
                    ?.trim()
